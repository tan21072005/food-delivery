...
public class OrderDetailFragment extends Fragment {
    ...
    private void renderOrderDetail(JsonObject detail) {
        ...
        JsonObject firstItem = firstOrderItem(detail);
        if (firstItem == null) {
        ...
        int quantity = (int) Math.max(1, Math.round(numberValue(firstItem, "quantity", "qty")));
        String itemName = firstString(firstItem, "item_name", "menu_item_name", "name");
        long itemPrice = Math.round(numberValue(firstItem, "line_total", "total_price", "base_price", "unit_price"));
        setText(tvFoodQuantityLabel, quantity + "x");
        setText(tvFoodNameLabel, itemName == null ? "Mon an" : itemName);
        setText(tvFoodPriceLabel, MoneyFormatter.format(itemPrice));
    }

    private JsonObject firstOrderItem(JsonObject detail) {
